<template>
  <form @submit="onSubmit">
    ...
    <div class="flex justify-center">
      <button v-if="hasPrevious" type="button" class="px-5 py-3 w-48 text-lg font-medium text-white bg-gray-900 hover:bg-gray-300 active:bg-gray-900 focus:outline-none border-4 border-white transition-all uppercase" @click="goToPrev">
        Previous
      </button>
      <button type="submit" class="px-5 py-3 w-48 text-lg font-medium text-white bg-blue-900 hover:bg-blue-300 active:bg-grey-900 focus:outline-none border-4 border-white transition-all uppercase">{{ isLastStep ? "Submit" : "Next" }}</button>
    </div>
  </form>
</template>
